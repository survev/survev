...
interface GridCollider {
    ...
    /**
     * whether this collider should contribute to obstacle density padding. it thus
     * only makes sense to set this to `true` if `type === "obstacle"`
     */
    useForPadding: boolean;
    defType: string;
}
...
export class GameMap {
    ...
    generateObjects(): void {
        ...
        for (const type in densitySpawns) {
            this.genDensitySpawn(type, densitySpawns[type] * 5);
        }
    ...
    genDensitySpawn(type: string, density: number) {
        ...
        // for objects that spawn only on river shores
     ...
     * @param doSpawn The callback to spawn it, it should return true on a successful spawn attempt to break the loop
    ...
    genOnGrass(type: string) {
        ...
        const getPos = () => {
            ...
            if (this.factionMode) {
                ...
                // doesn't matter which team, just as long as they're grouped with the team specific buildings
                const edgeObjects = [
                    "warehouse_01f",
                    "house_red_01",
                    "house_red_02",
        ...
        if (this.placeSpawns.includes(type)) {
            let attempts = 0;
