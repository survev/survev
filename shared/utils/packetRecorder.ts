...
export class PacketRecorder {
    ...
    private ensureCapacity(extraBytes: number) {
        const minByteLength = this.index + extraBytes;
        if (minByteLength <= this.view.byteLength) return;

        let nextByteLength = this.view.byteLength || 1;
        while (nextByteLength < minByteLength) {
            nextByteLength *= 2;
        }

        const nextBuffer = new ArrayBuffer(nextByteLength);
        new Uint8Array(nextBuffer).set(this.uint8Buff.subarray(0, this.index));
        this.uint8Buff = new Uint8Array(nextBuffer);
