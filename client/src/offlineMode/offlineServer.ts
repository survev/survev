...
import { Connection, ConnectionState } from "../../../shared/net/connection.ts";
...
export class Socket<T extends object = object> extends Connection {
    data = {} as T;
...
function createSocketPair() {
    ...
    clientSocket.send = (data) => {
        serverSocket.onMessage(
            data instanceof Uint8Array
                ? data.slice(data.byteOffset, data.byteLength).buffer as ArrayBuffer
                : data as ArrayBuffer,
        );
    };
    ...
    clientSocket.close = (reason?: string) => {
        serverSocket.onClose(0, reason ?? "");
        clientSocket.onClose(0, reason ?? "");
        serverSocket.data.closed = true;
    };
    serverSocket.close = (reason?: string) => {
        clientSocket.onClose(0, reason ?? "");
        serverSocket.onClose(0, reason ?? "");
        serverSocket.data.closed = true;
    };
    ...
    newGame(config: ServerGameConfig): GameWorker {
        ...
        for (let i = 0; i < count; i++) {
            setTimeout(() => {
                ...
                gameProc.addJoinToken([{ joinToken, ip: "", userId: null }]);
                const socket = this.connect(id);
                const bot = new Bot(i, socket, joinToken);
                this.bots.add(bot);
            }, 100 * i);
        }
        ...
        if (game.state !== ProcState.Running) {
            return new Promise<{ gameId: string; data: string }>((resolve) => {
                game.onCreatedCbs.push((game) => {
                    game.addJoinToken([{ joinToken, userId: "", ip: "" }]);
                    resolve({
