    ...
    init() {
        if (!this.initialized) {
            ...
            const rotateStep = (dir: number) => {
                ...
                const shift = dir > 0 ? isRotated : !isRotated;
                isRotated = !isRotated;

                if (shift) {
                    const active = this.loadout.emotes.slice(0, slotsCount);
                    const rest = this.loadout.emotes.slice(slotsCount);
                    ...
                    this.loadout.emotes = [...active, ...rest];
    ...
    refreshLoadoutWheel(slotsCount: number) {
        ...
        for (let i = 0; i < EmoteSlot.Count; i++) {
            ...
            if (i < slotsCount || isWinOrDeath) {
                ...
                this.equippedItems[i] = GameObjectDefs.typeExists(emote)
                    ? { type: emote } as EquippedItem
                    : {} as EquippedItem;
            }
        }
