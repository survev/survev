...
import { validateUserName } from "@survev/shared/utils/profanity";
import { HTTPRateLimit, WebSocketRateLimit } from "@survev/shared/utils/ratelimit";
import { assert } from "@survev/shared/utils/util";
import type { FindGameError } from "@survev/types/api";
import type { FindGamePrivateBody } from "@survev/types/misc";
import {
    type ClientRoomData,
...
import { hashIp, isBanned } from "./routes/private/ModerationRouter";
import { getHonoIp, isBehindProxy, verifyTurnsStile } from "./utils";
    ...
    async findGame(data: TeamPlayGameMsg["data"]) {
        ...
        if (this.data.captchaEnabled) {
            ...
            try {
                ...
                this.teamMenu.server.logger.error("Failed verifying turnstile:", err);
        ...
        for (const player of this.players) {
            ...
            if (!token) {
                this.teamMenu.server.logger.warn(
...
export class TeamMenu {
            ...
            upgradeWebSocket(async (c) => {
                ...
                if (sessionId) {
                    try {
                        ...
                        this.server.logger.error(`Failed to validate session:`, err);
                        userId = null;
                    }
                }
                ...
                return {
                    ...
                    onMessage(event, ws) {
                        ...
                        if (wsRateLimit.isRateLimited(data.rateLimit)) {
                            teamMenu.server.logger.warn("Rate limited, closing socket.");
                            ws.close();
                            return;
                        }
    ...
    onMsg(ws: WSContext<SocketData>, data: string) {
        ...
        try {
            ...
            this.server.logger.warn("Failed to parse message, closing socket.");
            ws.close();
            return;
        }
        ...
        if (!player) {
            this.server.logger.warn("Player not found, closing socket.");
            ws.close();
            return;
        }
        ...
        // so close the socket
        if (!player.room) {
            this.server.logger.debug("Player not in room, closing socket.");
            ws.close();
            return;
        }
    ...
    onClose(ws: WSContext<SocketData>) {
        ...
        if (!player) {
            this.server.logger.debug("Player not found, closing socket.");
            ws.close();
            return;
        }
